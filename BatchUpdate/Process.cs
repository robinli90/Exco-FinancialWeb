...
namespace BatchUpdate
{
    ...
    public class Process
    {
        ...
        // all runtime list
        public List<RunTime> allRuntimeList = new List<RunTime>();
        // exception index list in all runtime list
        public List<int> allExceptionList = new List<int>();
        // displayed runtime index list
        public List<int> displayedRuntimeIndexList = new List<int>();
        // displayed exception index list
        public List<int> displayedExceptionIndexList = new List<int>();
        // minimum so#
        ...
        public void CalculateRuntime()
        {
            foreach (Die die in dieMap.Values)
            {
                ...
                die.GetEvenTime(20, seqTemplateMap, seq20HJOBHMap);
                die.GetEvenTime(30, seqTemplateMap, seq30HJOBHMap);
        ...
        // fill displayed runtime/exception list
        public void FillDisplayList()
        {
            // fill runtime list
            pieceList.Sort(CompareWO);
            foreach (Piece piece in pieceList)
            {
                foreach (Seq seq in piece.seqList)
                {
                    foreach (RunTime runtime in seq.runtimeList)
                    {
                        ...
                        allRuntimeList.Add(runtime);
                    }
                }
            }
            for (int i = 0; i < allRuntimeList.Count; i++)
            {
                if (allRuntimeList[i].IsException())
                {
                    allExceptionList.Add(i);
                }
            }
        }
        ...
        public void ProcessPieces()
        {
            for (int i = 0; i < pieceList.Count; i++)
            {
                ...
                try
                {
                    piece.GetInfo(seqTemplateMap, scrapMap, sparkMap, wireMap, date);
        ...
        public void BackFlush()
        {
            foreach (Piece piece in pieceList)
            {
                piece.BackFlush(date, assemblyFinishMap);
            }
        }
        ...
        // mark complete for processed tasks
        public void MarkComplete()
        {
            foreach (RunTime runtime in allRuntimeList)
            {
                for (int j = 0; j < runtime.taskList.Count; j++)
                {
                    runtime.taskList[j].MarkComplete();
                }
            }
        }

        // generate cache content for batch update
        public void GenerateBatchCache()
        {
            foreach (RunTime runtime in allRuntimeList)
            {
                if (runtime.runTime > 0.00001 || runtime.actualTime > 0.0001)
                {
                    try
                    {
                        ...
                        runtime.ProcessEmployeeNumber();
                        if (!cacheMap.ContainsKey(runtime.resCode))
                        {
                            cacheMap.Add(runtime.resCode, runtime.GenerateBatchString());
                        }
                        else
                        {
                            cacheMap[runtime.resCode].AddRange(runtime.GenerateBatchString());
                        }
                    }
                    catch (Exception e)
                    {
